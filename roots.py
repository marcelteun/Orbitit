...
if __name__ == '__main__':
    ...
    faster = False
    #FoldedRegularHeptagonsS4A4(tmp, [0])
    #tmp = numx.array((1.4483279191685114, 1.4512124435056795, -1.7538347714390179, 1.7903783867922103))
    #tmp = numx.array((0.00, 0.00, 0.00, 0.00))
    #print FoldedRegularHeptagonsS4A4(tmp,
    #    [TriangleAlt.alt_stripII, [0., 0., 0., 0.], Fold.w ])
	...
	#    fold = Fold.star,
        ...
        #    # doesn't converge:
	#    # taken from 1, 1, 0, 1
        #    initialValues   = numx.array((1.4483279191685114, 1.4512124435056795, -1.7538347714390179, 1.7903783867922103)),
	#    edgeAlternative = TriangleAlt.alt_stripII,
	#    edgeLengths = [1., 0., 1., 0.],
	#    fold = Fold.w,
        #    #printIter = True,
        #    #method          = Method.hybrids
	...
	pass
        ...
        result = FindMultiRootOnDomain(Domain,
                #edgeAlternative = TriangleAlt.strip1loose,
                #edgeAlternative = TriangleAlt.stripI,
                #edgeAlternative = TriangleAlt.stripII,  # same as stripI, with edgelengths 0, 1, 1, 1 or 0, 1, V2, 1
                #edgeAlternative = TriangleAlt.star, # same as stripI, with edgelengths 1, 1, 1, 0
                #edgeAlternative = TriangleAlt.star1loose, # same as strip1loose, with edgelengths 1, 1, 1, 0
                #edgeLengths = [1., 1., 1., 1.],
                #edgeLengths = [1., 1., 1., 0.],
                #edgeLengths = [1., V2, 1., 0.], # only special for strip II and star
                #edgeLengths = [1., 1., V2, 1.], # only special for strip II and star
                # oops edgeLengths = [1., 1., V2, 0.],
                #edgeLengths = [V2, 1., V2, 1.],
                #edgeLengths = [1., V2, 1., 1.],
                #edgeLengths = [V2, 1., 1., 1.], # only Loose variants
                #edgeLengths = [0, 1., 1., 1.], # not for loose variants.
                #edgeLengths = [0, 1., V2, 1.], # not for loose variants.
                #edgeLengths = [0, 1., 1., 0.], # not for loose variants.
		...
		#--------------------------------------------------------------
		# Heptagons only:
		# prefered: for star, stripI:
                #edgeAlternative = TriangleAlt.star,
                #edgeLengths = [1., 0., 1., 0.],
		# valid for strip1loose, star1loose (doesn't work as well):
                #edgeAlternative = TriangleAlt.strip1loose,
                #edgeLengths = [0., 0., 1., 0.],
		# (stripII: use 1 Rho 1 0... though doesn't work out well..)
		#fold = Fold.w,
		#fold = Fold.trapezium, # no sol found step 0.3 [-2, 3]
		#fold = Fold.star, # no sol found step 0.3 [-2, 3]
		#fold = Fold.parallel,
		#fold = Fold.triangle, # no sol found step 0.3 [-2, 3]

		# Another heptagons only?

		#--------------------------------------------------------------
		# only o3 triangles:
		# strip I, strip II: 0, 1, 0, 1
                #edgeAlternative = TriangleAlt.stripI,
                #edgeLengths = [0., 1., 0., 1.],
		#fold = Fold.trapezium,
		#fold = Fold.w,
		#fold = Fold.star,
		#fold = Fold.triangle,
		#fold = Fold.parallel, # none found
		# or  star 1 loose: Rho, 1, Rho, 1
		# or  strip 1 loose: Rho, 1, 0, 1
		# or  star: 0, 1, Rho, 1
		...
		# no o3 triangles:
                #edgeAlternative = TriangleAlt.strip1loose,
                #edgeLengths = [1., 1., 1., 0.],
		#fold = Fold.parallel, # done
		# TODO
		#fold = Fold.trapezium,

		#--------------------------------------------------------------
		# only pair 1 triangles:
		# star = stripI = strip II
                #edgeAlternative = TriangleAlt.star,
                #edgeLengths = [0., 1., 1., 0.],
		#fold = Fold.parallel, # done
		#for
		...
		# If d == 0 (or no o3 triangles) there is no way to
		# get 1 pair of triangles only: either all
		# will disappear or 2 pairs will be there..
		# fold = Fold.triangle, # done: 3 solutions found

                # TODO check for more loose variants:
		# e.g. strip1loose == star1loose:
                #edgeAlternative = TriangleAlt.star1loose,
                #edgeLengths = [0., 1., 1., 0.],
		#fold = Fold.trapezium, # none found
		#                       # close at 2.5, 0.51, 2.97, -1.43

		#--------------------------------------------------------------
		# Square and 1 kind of triangle, no O3
		# TODO
                #edgeAlternative = TriangleAlt.star, # found 4 solutions
                #edgeAlternative = TriangleAlt.strip1loose, #
                #edgeLengths = [V2, 1., 1., 0.],
				   # similarly: 1, V2, 1, 0, for loose,..
		# fold = Fold.star # 4 solutions found
		#fold = Fold.parallel, # 4 solutions found
		...
		# Only hepts
                ...
                edgeLengths = [1., 0., 1., 0.],
		fold = Fold.w, # 
		# TODO what about other folds?

		#--------------------------------------------------------------
		# bordering pair and O3 tris: (Completed)
		# stripI and stripII share solutions
		# strip1loose and star1loose share solutions
		# star tris: nothing special. Nothing similar.
		# star tris: try [1, 0, 1, 1] (no border triangles)
		# 	close at (2.00, 1.06,  0.58,  0.12) but no conversion
		# essentially different (same solutions for star1loose.
                #edgeAlternative = TriangleAlt.stripI,
                #edgeAlternative = TriangleAlt.strip1loose,
                #edgeLengths = [1., 1., 0., 1.],
		# strip I
		#fold = Fold.trapezium, # none found
		#fold = Fold.triangle, # 4 solutions found
		#fold = Fold.star, # 7 solutions found
		#fold = Fold.parallel, # no solutions found
		#fold = Fold.w, # 5 solutions found
		# strip1loose:
		#fold = Fold.star, # 7 solutions found
		#fold = Fold.w, # # 7 solutions found
		#fold = Fold.triangle, # 8 solutions found
		#fold = Fold.parallel, #  no solutions found
		#fold = Fold.trapezium, #  no solutions found

		#--------------------------------------------------------------
		#TODO
		# the above can be transformed into squares and O3
		# V2, 1, 0, 1....
		# triangle alt strip I == strip II
		# nothing special for triangle alt star
                #edgeAlternative = TriangleAlt.stripI,
                #edgeLengths = [V2, 1., 0., 1.],
		#fold = Fold.star, # 6 solutions found
		#fold = Fold.w, # 4 solutions found
		#fold = Fold.triangle, # 6 solutions found
		#fold = Fold.parallel, # no solutions found
		#fold = Fold.trapezium, # no solutions found
		# TODO: loose varaiants
		#continueTestAt = [-2.0, -1.3415926535897935, -0.14159265358979345, -0.74159265358979343],
		#results = [],

		#--------------------------------------------------------------
		# xxx and O3 tris:
		# star tris: try [1, 0, 1, 1]
		# 	e.g. (2.00, 1.06,  0.58,  0.12)
		# essentially different (same solutions for star1loose.
#                edgeAlternative = TriangleAlt.star, # others: TODO
#                edgeLengths = [1., 1., 0., 1.],
		#fold = Fold.trapezium, # TODO
		#fold = Fold.triangle, # TODO
#		fold = Fold.star,
		#fold = Fold.parallel, # TODO
		#fold = Fold.w, # TODO

		#--------------------------------------------------------------
		# Only 12 folded squares:
		# [V2, 1, 0, 0]
		# strip I:
                #edgeAlternative = TriangleAlt.stripI,
		...
		#fold = Fold.star, #
                ...
                stepSize = 0.3,
                maxIter = 20
		#,continueTestAt = [-2.0, -3.1415926535897931, 2.2584073464102068, 0.45840734641020653]
            )
        print '['
