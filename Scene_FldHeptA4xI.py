...
class CtrlWin(wx.Frame):
    ...
    def createControlsSizer(this):
        ...
        mainVSizer.Add(statSizer, 3, wx.EXPAND)
        mainVSizer.Add(this.specPosSizerV, 7, wx.EXPAND)
    ...
    def onPrePos(this, event = None):
        ...
        if sel != 0:
            ...
            if selStr == this.prePosLst[i]:
                # 1, V2, 1, 1
                EnsureInRange()
                if not (triangleAlt == c.T_STAR_1_LOOSE or triangleAlt == c.T_STRIP_1_LOOSE):
                    tVal = this.specPos[i][triangleAlt][this.specPosIndex][0]
                    aVal = this.specPos[i][triangleAlt][this.specPosIndex][1]
                    fld1 = this.specPos[i][triangleAlt][this.specPosIndex][2]
                    fld2 = this.specPos[i][triangleAlt][this.specPosIndex][3]
            ...
            if selStr == this.prePosLst[i]:
                # V2, 1, 1, 1
                EnsureInRange()
                if (triangleAlt == c.T_STAR_1_LOOSE or triangleAlt == c.T_STRIP_1_LOOSE):
                    print this.specPosIndex, 'of', len(this.specPos[i][triangleAlt])
                    for s in this.specPos[i][triangleAlt][this.specPosIndex]:
                        print '  o ', s
                    tVal = this.specPos[i][triangleAlt][this.specPosIndex][0]
                    aVal = this.specPos[i][triangleAlt][this.specPosIndex][1]
                    fld1 = this.specPos[i][triangleAlt][this.specPosIndex][2]
...
#                fld2 = -2.775882363308309
            ...
            if (
                    (
                        selStr == '|a|=1 & |b|=1 & |c|=1 & |d|=0'
                        and triangleAlt == c.T_STRIP_II
                    )
                    or
                    (
                        selStr == '|a|=1 & |b|=1 & |c|=1 & |d|=1'
                        and triangleAlt == c.T_STRIP_II
                    )
                    or
                    (
